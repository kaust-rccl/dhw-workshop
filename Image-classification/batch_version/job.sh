...
do
    srun --cpu-bind=cores -n 1 -N 1 -c ${SLURM_CPUS_PER_TASK} \
    -w ${nodes_array[i]} --gpus=${SLURM_GPUS_PER_NODE}  \
    python -m torch.distributed.launch --use_env \
    --nproc_per_node=${SLURM_GPUS_PER_NODE} \
    --nnodes=${SLURM_NNODES} --node_rank=${i} \
    --master_addr=${master_ip} --master_port=${MASTER_PORT}  ../scripts/train_resnet50_ds.py \
    --epochs 5 --num-workers=${SLURM_CPUS_PER_TASK}\
