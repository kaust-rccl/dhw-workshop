...
cd ../testCase-2 || { echo "Error: 'testCase-2' directory not found!"; exit 1; }
...
echo "Cleaning previous simulation files and processor directories..."
find . -maxdepth 1 -type d -regex './[0-9]+' -not -name '0.org' -exec rm -rf {} +  # Removes directories like 0, 1, 2, etc., but keeps 0.org
...
# Initialize the fields using funkySetFields
echo "Initializing fields..."
funkySetFields -time 0 > log.funkySetFields 2>&1
if [ $? -ne 0 ]; then
    echo "Error: Field initialization with funkySetFields failed. Check log.funkySetFields for details."
    exit 1
fi
...
mpirun -np "$NUM_PROCESSORS" myConvectionDiffusionFoam > log.simulation 2>&1
if [ $? -ne 0 ]; then
